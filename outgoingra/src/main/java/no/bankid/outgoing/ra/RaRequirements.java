...
public interface RaRequirements {
    ...
    @Operation(summary = "Get customer service contact details for booking an appointment",
            description = "Returns contact information for the financial institution's customer service to allow the user to book an appointment for the catch all activation method.",
    ...
    @Operation(summary = "Check if user identity has been verified by RA officer",
            description = "Validates the verification code provided by the RA Officer as part of the catch all activation method and marks it as consumed (single-use). The verification code is generated by the bank's RA system, and provisioning the code to the user is the bank's responsibility. Returns verified=true on success (code is consumed), or verified=false with an error code on failure.",
            tags = {APP_ENROLLMENT_IDENTIFICATION_BY_RA_OFFICER})
